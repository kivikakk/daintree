...
fn handle(ctx: *ExceptionContext, comptime name: []const u8) callconv(.Inline) noreturn {
    entry.uart.carefully(.{"exception handler running for " ++ name ++ "\r\n"});
...
fn dumpRegs(ctx: *ExceptionContext) void {
    entry.uart.carefully(.{ "ctx ptr:", @ptrToInt(ctx), "\r\n" });
    entry.uart.carefully(.{ "x0  ", ctx.regs[0], "  x1  ", ctx.regs[1], "\r\n" });
    entry.uart.carefully(.{ "x2  ", ctx.regs[2], "  x3  ", ctx.regs[3], "\r\n" });
    entry.uart.carefully(.{ "x4  ", ctx.regs[4], "  x5  ", ctx.regs[5], "\r\n" });
    entry.uart.carefully(.{ "x6  ", ctx.regs[6], "  x7  ", ctx.regs[7], "\r\n" });
    entry.uart.carefully(.{ "x8  ", ctx.regs[8], "  x9  ", ctx.regs[9], "\r\n" });
    entry.uart.carefully(.{ "x10 ", ctx.regs[10], "  x11 ", ctx.regs[11], "\r\n" });
    entry.uart.carefully(.{ "x12 ", ctx.regs[12], "  x13 ", ctx.regs[13], "\r\n" });
    entry.uart.carefully(.{ "x14 ", ctx.regs[14], "  x15 ", ctx.regs[15], "\r\n" });
    entry.uart.carefully(.{ "x16 ", ctx.regs[16], "  x17 ", ctx.regs[17], "\r\n" });
    entry.uart.carefully(.{ "x18 ", ctx.regs[18], "  x19 ", ctx.regs[19], "\r\n" });
    entry.uart.carefully(.{ "x20 ", ctx.regs[20], "  x21 ", ctx.regs[21], "\r\n" });
    entry.uart.carefully(.{ "x22 ", ctx.regs[22], "  x23 ", ctx.regs[23], "\r\n" });
    entry.uart.carefully(.{ "x24 ", ctx.regs[24], "  x25 ", ctx.regs[25], "\r\n" });
...
export fn el1_sync(ctx: *ExceptionContext, elr_el1: u64, esr_el1: u64) void {
    ...
    entry.uart.hex(elr_el1);
    entry.uart.hex(esr_el1);
    handle(ctx, "EL1_SYNC");
}
